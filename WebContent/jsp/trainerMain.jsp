...
<script src="js/scriptTrainerMain.js">
...
<body>
	<div class="trainerMain">
		<div class="trainerMain-left" style="position: relative; height: 550px;">
			<div style="width: 50%">
			...
			<div align="center"><font size="5">${member_name}</font>님의,</div>
			
			
			<c:if test="${!empty bodyinfo_key}">
        	<font color="red" style="font-weight: bold;">[신체정보]</font><br>
			몸무게(${weight}kg), 키(${height}cm), 체지방률(${fat_rate}%), 근육량(${muscle_rate}%), 수분량(${water_rate}%), 비만도(${mass_index}%) <br>
   			</c:if>
	
			<c:if test="${!empty exercise_key}">
			<font color="red" style="font-weight: bold;">[운동세트]</font><br>${content} <br>
			</c:if>
	
			<c:if test="${!empty daymenu_key}">
			<font color="red" style="font-weight: bold;">[식단]</font><br>아침(${morning}), 점심(${lunch}), 저녁(${dinner}) <br>
			</c:if>
			
			<c:if test="${!empty message_key}">
			<font color="red" style="font-weight: bold;">[메시지]</font><br>${message}
			</c:if>		

    		<c:if test="${empty bodyinfo_key && empty exercise_key && empty daymenu_key && empty message_key}">
      		<br><br><br><div align="center">오늘 정보가 없습니다</div><br><br><br>
    		</c:if>	
			<div style="position:absolute; bottom:10px; margin-left: 25%;">
				<br><br><div align="center"><a href = "trainerMainFirst.do"><font color="orange">다른 회원 선택하러 가기</font></a></div>
				<br><div align="center"><a href="logout.do">로그아웃</a></div>
			</div>
			
			</ul>
		</div>

		<div class="trainerMain-right">
			<div id="container">
				<ul class="tabs">
					<li id="menutab1" class="active" rel="tab1">신체</li>
					<li id="menutab2" rel="tab2">운동</li>
					<li id="menutab3" rel="tab3">식단</li>
